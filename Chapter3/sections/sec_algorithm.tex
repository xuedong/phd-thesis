...
Recall that we have closed-form expressions for the \texttt{KL}-divergence in some cases. In particular, in the Gaussian case, $d(\mu;\mu') = (\mu-\mu')^2/(2\sigma^2)$ while in the Bernoulli case $d(\mu;\mu') = \mu \ln (\mu/\mu') + (1-\mu)\ln (1-\mu)/(1-\mu')$.
...
Besides, we let $\Sigma_K = \{\bomega : \sum_{k=1}^K \omega_k = 1\}$ be the probability simplex of dimension $K$.
