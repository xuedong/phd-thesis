...
%\todo[inline]{ We prove that $d_{t,\delta} \geq T_t^{\istar} \Tstar(\btheta)^{-1}$ and that $T_t^i = O(\sqrt{t})$ ,for all $i\neq \istar$}
...
%\ge \frac{1}{2}\sum_{i\in\cI} \inf_{\btheta'_i \in \neg i}\Vert \hat{\btheta}_t - \btheta'_i \Vert_{\bLambda_{T_t^i}}^2
\: ,
\end{align*}
...
%\ge \Vert \btheta - \btheta'_{i,T_t^i}(\btheta) \Vert_{\bLambda_{T_t^i}}
\: .
\end{align*}
We obtain
\begin{align*}
...
% &\le 2 C_t \sum_s \sum_{i,a} w_s^{i,a} a^\top \bLambda_{T_s^i}^{-1}a + 2\sqrt{2 C_t}\sqrt{\sum_s \sum_{i,a} w_s^{i,a}a^\top \bLambda_{T_s^i}^{-1}a }\sqrt{\sum_s \sum_{i,a} w_s^{i,a}\Vert \hat{\btheta}_{s-1} - \btheta'_{s,i}\Vert_{\bx\bx\transpose}^2}
\end{align*}
