...
Note that rewards of unchosen arms at each time step are not revealed: this partial feedback setting is thus a special case of the online learning with experts setting.
\end{leftbar}
\end{remark}
...
When the search space is continuous, it is the \gls{go} or \gls{bbo} problem. Sometimes we can also refer to \gls{zo}, in contrast to \emph{first-order optimization} for which gradient-based information is available.
