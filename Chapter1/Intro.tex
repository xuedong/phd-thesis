...
We start by an introductory Chapter~\ref{chap:mab} about the MAB model. We recall some basic notions as well as some fundamental results for stochastic MAB for the sake of integrity. We pay particular attention to how the settings previously mentioned in Section~\ref{sec:intro.mab} are formally defined.

Chapter~\ref{chap:t3c} is dedicated to the problem of stochastic best-arm identification.

Chapter~\ref{chap:lgc} studies best-arm identification for linear bandits.

Chapter~\ref{chap:gpo} is dedicated to continuum-armed bandits.

Finally, we devote a whole Chapter~\ref{chap:dttts} to \gls{automated machine learning} before concluding in Chapter~\ref{chap:conclusion}. We focus in particular on hyper-parameter optimization in this chapter.
